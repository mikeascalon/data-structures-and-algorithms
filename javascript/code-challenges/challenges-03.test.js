...
const typeNum = (arr) => {
  let onlyNum = arr.filter((value) => typeof value === 'number');
  return onlyNum;
...
const containsAnd = (arr) => {
  let stringHaveAnd = arr.filter((value) => value.includes('and'));
  return stringHaveAnd;
};
...
const oddValues = (arr) => {
  let oddnum = arr.filter( (value) => value % 2 === 1);
  return oddnum;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notInFirst = arr.filter((value)=> !forbiddenValues.includes(value));
  return notInFirst;
};
